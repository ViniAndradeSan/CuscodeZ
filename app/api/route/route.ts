...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    console.error("[voice] GROQ_API_KEY não encontrada no ambiente");
    return NextResponse.json({ error: "Serviço indisponível" }, { status: 500 });
  }
  console.log("[voice] apiKey encontrada, tamanho:", apiKey.trim().length);
  ...
  console.log("[voice] Chamando Groq com modelo llama-3.3-70b-versatile");
  ...
  console.log("[voice] Groq respondeu status:", response.status);
  if (!response.ok) {
    const errText = await response.text().catch(() => "");
    console.error("[voice] Groq erro:", response.status, errText);
    return NextResponse.json({ error: "Serviço indisponível" }, { status: 500 });
  }
